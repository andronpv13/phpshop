...
function actionStart() {
    ...
    // SQL
    $PHPShopOrm = new PHPShopOrm($PHPShopModules->getParam("base.pricerequest.pricerequest_jurnal"));
    ...
    if (is_array($data))
        foreach ($data as $row) {
            ...
            $PHPShopInterface->setRow($row['id'], array('name' => $icon, 'link' => '?path=modules.dir.pricerequest&id=' . $row['id'], 'align' => 'left'), array('name' => $row['product_name'], 'link' => '?path=modules.dir.pricerequest&id=' . $row['id']),$row['product_price'].' '.$currency,array('name' => $row['name'], 'link' => '?path=modules.dir.pricerequest&id=' . $row['id'],'addon'=>$mail), PHPShopDate::get($row['date'], false),   array('action' => array('edit', '|', 'delete', 'id' => $row['id']), 'align' => 'center'), '<span style="color:'.$status_array[$row['status']]['color'].'">'.$status_array[$row['status']]['name'].'</span>');
        }
