    ...
    if (is_array($data)) {
        ...
        foreach ($sort_data as $k => $row) {
            ...
            if (in_array(pathinfo($name, PATHINFO_EXTENSION), ['mp4', 'mov'])) {
                $bxslider .= '<div><video class="embed-responsive-item" src="'.$name.'" controls autoplay loop style="max-width:100%; height:auto"></video></div>';
                $bxsliderbig .= '<li><div class=\'embed-responsive embed-responsive-4by3\'><video class=\'embed-responsive-item\' src=\''.$name.'\' controls autoplay loop  style=\'max-height:500px\'></video></div></li>';
